...
\title{HIREewas: Detection Cell-Type-Specific Risk-CpG Sites in EWAS\\ User's Guide}
...
\noindent Here we simulated a methylation matrix \Robject{Ometh} with 10,000 CpG sites and 300 samples as well as a covariate matrix \Robject{X} with one binary covariate (case/control) in the first row and one continuous variable (age) in the second row. We can further look into the details.\\
...
par(mfrow=c(1,3))
plot(ret_list$P_t[3, ], P[1, ], xlim=c(0,1), ylim=c(0,1))
abline(a=0, b=1, col="red")

plot(ret_list$P_t[1, ], P[2, ], xlim=c(0,1), ylim=c(0,1))
abline(a=0, b=1, col="red")

plot(ret_list$P_t[2, ], P[3, ], xlim=c(0,1), ylim=c(0,1))
abline(a=0, b=1, col="red")
@  
...
<<>>=
riskCpGpattern(ret_list$pvalues[1:100, K+c(3,1,2)], 
...
%\noindent In practice, the underlying cell type number might be unknown to us. In this case, we can resort to the pBIC criterion to select the cell type number by try multiple choices of the cell type number.\\

%<<>>=
%pBIC <- NULL
%for(k in 2:4){
%	ret_list <- HIRE(Ometh, X, num_celltype=K, tol=10^(-4), num_iter=1000, alpha=0.01)
%	#only use the first 1000 CpG sites for a quick demonstration
%	pBIC <- c(pBIC, ret_list$pBIC)
%}
%plot(2:4, pBIC, xlab = "cell type number", ylab = "pBIC", main = "pBIC plot", type="b")
%@ 

%\noindent As you can see, when the cell type number is three, the pBIC value attains the minimum, thus correctly recovering the underlying truth.\\ 
